...
def generate_time_options(time_limit):
    time = [0]
    time_step = time_limit //2 
    time.append(random.randint(0,time_step))
    time.append(random.randint(0,time_step))
    time.append(random.randint(time_step, time_limit))
    time.append(random.randint(time_step, time_limit))
    return time
...
def single_step_modify(packet_matrix, surogate_model, sequence_nums,
    ...
    time_options = generate_time_options(100) 
    og_loss = float(surogate_model(torch.tensor(packet_matrix)))
    best_loss = og_loss
    non_zero_best_time_count = 0 
    ...
    for modify_round in range(modify_rounds):
        ...
        for packet_num in range(len(cur_selections)):
            ...
            og_time = original_packet_matrix[packet_num][surogate_model.PKT_TIME]
            for i, option in enumerate(flow_options[packet_num]):
                for time_pert in time_options:
                    option_numpy_matrix[packet_num][surogate_model.PKT_LEN] = original_val + 4 * option
                    option_numpy_matrix[packet_num][surogate_model.PKT_TIME] = og_time + time_pert
            ...
            numpy_packet_matrix[packet_num][surogate_model.PKT_TIME] = og_time + best_time
            if best_time != 0:
                non_zero_best_time_count += 1
        if (numpy_packet_matrix == prev_numpy_matrix).all():
            break
        prev_numpy_packet_matrix = numpy_packet_matrix.copy()
    print("Loss {:5.5f} -> {:5.5f}, Num-Nonzero: {}".format(og_loss, best_loss, non_zero_best_time_count))
    return flow_options, best_options, best_times, non_zero_best_time_count
...
def make_modifications(flow_options, best_selections, sequence_nums, packet_lookup, write_queue, best_times):
    """
...
def disguise_traffic(pcap_loc, label_loc, clusters_loc, result_loc):
    ...
    for (src_addr, first_timestamp,
        ...
        if matrix_label == 0 or packet_matrix.shape[0] < 1188:
            ...
            flow_options, best_selections, best_times, non_zero_counts = single_step_modify(packet_matrix, surogate_model,
                                                               sequence_nums, packet_lookup)
            time_perts += non_zero_counts
            make_modifications(flow_options, best_selections, sequence_nums, packet_lookup, write_queue, best_times)
    ...
    print("Total non-zero best times count: ", time_perts)  # Print total count
